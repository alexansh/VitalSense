...
fun DispensaryStockScreen(stock: List<DispensaryItem>) {
    Column(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        Text(stringResource(R.string.dispensaryStock), style = MaterialTheme.typography.headlineMedium)
        LazyColumn {
            items(stock) { item ->
                VitalSenseCard {
                    Column {
                        ...
                        if (item.isLowStock) Text(stringResource(R.string.lowStockAlertBadge), color = MaterialTheme.colorScheme.error)
                    }
                }
            }
        }
